...
Y = tf.placeholder(dtype=tf.float32, name='Life_Expectancy')
...
Y_predicted = w * tf.square(X) + u * X + b
...
plt.plot(data[:, 0], data[:, 0] ** 2 * w_out +
         data[:, 0] * u_out + b_out, 'r', label='Predicted data')
plt.legend()
plt.show()